...
import questionsDataQuizB from "../Data/QuestionsData/QuestionDataQuizB";
...
function QuestionsEnd({ unitNumber: unitProp }) {
  ...
  const [attempt] = useState(
    () => Number(sessionStorage.getItem("quiz_attempt_5")) || 1,
  );
  ...
  const handleDebugScenario = (scenario) => {
    const quizData =
      scenario.attempt >= 2 ? questionsDataQuizB : questionsDataQuiz;

    sessionStorage.setItem("finalQuizScore", scenario.score);
    sessionStorage.setItem("quiz_attempt_5", scenario.attempt);
    sessionStorage.setItem("unit_5_questions", JSON.stringify(quizData));
    sessionStorage.setItem(`unit_5_quiz_answers`, JSON.stringify({}));
