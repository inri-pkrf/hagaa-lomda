...
function GoalsBase({ unitKey, data }) {
    ...
    // פונקציה שמחזירה מערך של שורות לפי כמות המטרות
    function getGoalRows(goals) {
        ...
        // ברירת מחדל: שורה אחת
        return [goals.map((_, i) => i)];
    }

    const goalRows = getGoalRows(data.goals);
    ...
    return (
        <div className="Goals" style={{
            ...
            <p className="subtitles goals-subtitle">{data.subtitle}</p>

            <div className="goals-list-custom">
                {goalRows.map((row, rowIdx) => (
                    <div className={`goals-row goals-row-${row.length}`} key={rowIdx}>
                        {row.map((goalIdx) => (
                            <div
                                key={goalIdx}
                                className={`goal-item ${visibleGoals.includes(goalIdx) ? 'visible' : ''}`}
                            >
                                <div className="goal-cloud" style={{ borderColor: colors.main }} />
                                <span className="goal-text">{data.goals[goalIdx]}</span>
                            </div>
                        ))}
                    </div>
                ))}
