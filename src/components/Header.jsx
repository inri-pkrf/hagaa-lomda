...
const Header = () => {
  ...
  const hideProgressBarPaths = ["/", "/info-lomda", "/Structure", "/units-division", "/elevator", "/questions-end/5", "/last-page"];
  const shouldShowProgressBar = !hideProgressBarPaths.includes(location.pathname);

  const getTitle = (pathname, unit) => {
    for (const unitData of Object.values(TitlesData)) {
      if (unitData.pages?.[pathname]) return unitData.pages[pathname];
    }
    return TitlesData[unit]?.default || "שיעור הסמכה דיגיטלי";
  };
  ...
  useEffect(() => {
    ...
    window.addEventListener('updateTotalProgress', () => setCurrentUnit(prev => prev));
    return () => {
      clearInterval(interval);
      window.removeEventListener('updateTotalProgress', () => setCurrentUnit(prev => prev));
    };
  }, [currentUnit]);
  ...
  const unitInfo = unitNumber !== null && NavBarData[unitNumber - 1]
    ? { ...NavBarData[unitNumber - 1], unitNumber }
    : null;

  // ⭐ צבע הרקע — אם UnitZero (או כל יחידה ללא NavBarData) לוקח מ-headerData
  const isAboutUs = location.pathname === '/AboutUs' || location.pathname === '/CreditPage';


  const bgColor = isAboutUs
    ? "#fca6d4"
    : unitInfo?.color ?? headerData[currentUnit]?.backgroundColor ?? '#CACACA';
  ...
  return (
    <header className="app-header-0" style={{
