    ...
    const saveToServer = async () => {
      try {
        ...
        const res = await fetch('/umbraco/surface/learning/SetIframeLearning', {
          ...
          body: JSON.stringify({ lastPath: '/last-page', score, pass: score >= 70, sessionState }),
        });
    ...
    <div className="lastPage">
      ...
      <div className="lastPage__center">
        ...
        {isPass && (
          <>
            <h2 className="lastPage__subtitle">
              הציון עבר את הרף הנדרש לצורך קבלת תעודה,
              התעודה תחכה לך באיזור האישי
            </h2>
        ...
        {isFail && (
          <>
            <h1 className="lastPage__subtitle">לא עברת את הרף הנדרש לצורך קבלת תעודה</h1>

            {isFirstTry && (
              <>
                <h2 className="lastPage__subtitle_restart">
                  שימו לב: ניתן לנסות לגשת למבחן בשנית, אך לאחר מכן אם לא תעברו את המבחן תצטרכו להתחיל את הלומדה מחדש
                </h2>
                <button className="lastPage__button" onClick={() => setOpenReview(true)}>
                  איפה טעיתי
                </button>
      ...
      {openReview && (
        <div className="modalOverlay" onClick={() => setOpenReview(false)}>
          <div className="modalContent" onClick={(e) => e.stopPropagation()}>
            <button className="modalCloseBtn" onClick={() => setOpenReview(false)}>✕</button>
            <h2>סקירת טעויות</h2>
