...
function Chemical() {
    ...
    () => [
      {
        ...
        path: "/InfoChemical",
        completionKey: "chemicalFrame1Completed",
      },
      {
        id: 2,
        text: (
        ...
        path: "/CausesChemical",
        completionKey: "chemicalFrame2Completed",
      },
      {
        id: 3,
        text: (
        ...
        completionKey: "chemicalFrame3Completed",
      },
      {
        id: 4,
        text: <>הנחיות</>,
        path: "/GuidelinesChemical",
        completionKey: "chemicalFrame4Completed",
      },
      {
        id: 5,
        text: <>סיכום</>,
        path: "/summary-checklist-unit2-sub3",
        completionKey: "chemicalFrame5Completed",
      },
    ],
    [],
  );
  ...
  const [hasPlayedIntro] = useState(() => {
    return sessionStorage.getItem("chemicalIntroPlayed") === "true";
  });

  const [startSequence, setStartSequence] = useState(hasPlayedIntro);

  const [unlockedStep, setUnlockedStep] = useState(() => {
    const savedStep = sessionStorage.getItem("chemicalUnlockedStep");
    return savedStep ? parseInt(savedStep, 10) : 1;
  });

  useEffect(() => {
    rocketFramesData.forEach((frame) => {
      if (sessionStorage.getItem(`chemical-visited-${frame.id}`) === "true") {
        sessionStorage.setItem(frame.completionKey, "true");
        sessionStorage.setItem(`chemical-sub${frame.id}-finished`, "finished");
      }
    });

    const updated = getCompletedFrames();
    setClickedFrames(updated);
    sessionStorage.setItem("clickedChemicalFrames", JSON.stringify(updated));
    ...
    if (newUnlocked > unlockedStep) {
      ...
      sessionStorage.setItem("chemicalUnlockedStep", String(newUnlocked));
    }
  }, [location.pathname]);

  useEffect(() => {
    sessionStorage.setItem("MainTitle", "חומרים מסוכנים");
    if (!hasPlayedIntro) {
      const sequenceTimeout = setTimeout(() => {
        setStartSequence(true);
        sessionStorage.setItem("chemicalIntroPlayed", "true");
      }, 500);
