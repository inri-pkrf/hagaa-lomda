...
function States() {
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(
      new CustomEvent("setNextBtnDisabled", { detail: !isAllOpened }),
    );
  ...
  useEffect(() => {
    if (isAllOpened && !isStatesDone) {
      ...
      sessionStorage.setItem(
        "currentChapter",
        JSON.stringify({ name: "unitOne-second", state: "finished" }),
      );
      setCompleted(true);
  ...
  const handleCardClick = (id) => {
    ...
    sessionStorage.setItem(
      "statesOpenedCards",
      JSON.stringify(Array.from(newOpened)),
    );
    openStateCard(id);
  };
  ...
  return (
    <div className="threats-container">
      ...
      {!showCards && (
        <div
          className={
            isStatesDone ? "click-div-states-done" : "click-div-states"
          }
        ...
        <>
          ...
          <div className="board-container-wrapper">
            ...
            <img
              ...
              src={`${process.env.PUBLIC_URL}/assets/UnitOneImgs/StatesArtBoard.jpg`}
              alt="Board"
            />
      ...
      {showCardPopup && selectedId != null && (
        <StatesCard {...StatesData[selectedId]} onClose={closeStateCard} />
      )}
    </div>
  );
}
