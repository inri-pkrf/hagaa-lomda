...
function Resources() {
    ...
    () => [
      {
        ...
        path: "/ResourcesInfo",
        completionKey: "resourcesFrame1Completed",
      },
      {
        id: 2,
        text: (
        ...
        path: "/ResourcesGame",
        completionKey: "resourcesFrame2Completed",
      },
      {
        id: 3,
        text: (
        ...
        completionKey: "resourcesFrame3Completed",
      },
    ],
    [],
  );
  ...
  useEffect(() => {
    rocketFramesData.forEach((frame) => {
      if (sessionStorage.getItem(`resources-visited-${frame.id}`) === "true") {
        sessionStorage.setItem(frame.completionKey, "true");
        sessionStorage.setItem(`Resources-sub${frame.id}-finished`, "finished");
      }
    });

    const updated = getCompletedFrames();
    setClickedFrames(updated);
    sessionStorage.setItem("clickedResourcesFrames", JSON.stringify(updated));
    ...
    if (newUnlocked > unlockedStep) {
      ...
      sessionStorage.setItem("resourcesUnlockedStep", String(newUnlocked));
    }
  }, [location.pathname]);
  ...
  const handleFrameClick = (frame) => {
    if (frame.id <= unlockedStep) {
      sessionStorage.setItem(`resources-visited-${frame.id}`, "true");
      if (frame.id === unlockedStep && unlockedStep < rocketFramesData.length) {
        const next = unlockedStep + 1;
        setUnlockedStep(next);
        sessionStorage.setItem("resourcesUnlockedStep", String(next));
      }
      navigate(frame.path);
    }
  };
