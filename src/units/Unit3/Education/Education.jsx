...
function Education() {
    ...
    () => [
      {
        ...
        path: "/EducationInfo",
        completionKey: "educationFrame1Completed",
      },
      {
        id: 2,
        text: (
        ...
        path: "/EducationGame",
        completionKey: "educationFrame2Completed",
      },
      {
        id: 3,
        text: (
        ...
        completionKey: "educationFrame3Completed",
      },
    ],
    [],
  );
  ...
  useEffect(() => {
    rocketFramesData.forEach((frame) => {
      if (sessionStorage.getItem(`education-visited-${frame.id}`) === "true") {
        sessionStorage.setItem(frame.completionKey, "true");
        sessionStorage.setItem(`Education-sub${frame.id}-finished`, "finished");
      }
    });

    const updated = getCompletedFrames();
    setClickedFrames(updated);
    sessionStorage.setItem("clickedEducationFrames", JSON.stringify(updated));
    ...
    if (newUnlocked > unlockedStep) {
      ...
      sessionStorage.setItem("educationUnlockedStep", String(newUnlocked));
    }
  }, [location.pathname]);
  ...
  const handleFrameClick = (frame) => {
    if (frame.id <= unlockedStep) {
      sessionStorage.setItem(`education-visited-${frame.id}`, "true");
      if (frame.id === unlockedStep && unlockedStep < rocketFramesData.length) {
        const next = unlockedStep + 1;
        setUnlockedStep(next);
        sessionStorage.setItem("educationUnlockedStep", String(next));
      }
      navigate(frame.path);
    }
  };
