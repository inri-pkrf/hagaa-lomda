...
function GameLegalSituation() {
  ...
  const [placedAnswers, setPlacedAnswers] = useState({ attackHour: null, specialSituation: null });
  ...
  useEffect(() => {
    if (isGameFinished) {
      const alreadyFinished = sessionStorage.getItem('unitFour-first') === 'finished';
      if (!alreadyFinished) {
  ...
  return (
    <div className="GameLegalSituation-root">
      ...
      {/* Finish message */}
      {isGameFinished && (
        <div className="finish-message">כל הכבוד! סיימת בהצלחה את התרגול 🎉</div>
      )}
