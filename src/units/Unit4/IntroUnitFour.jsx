...
function IntroUnitFour() {
  ...
  const [doorImage, setDoorImage] = useState(`${process.env.PUBLIC_URL}/assets/General/Doors/unit4/bg-doors.jpg`);
  ...
  useEffect(() => {
    ...
    const updateFinished = () => {
      setFinishedChapters({
        ...
        unitFourSecond: sessionStorage.getItem('unitFour-second') === 'finished',
  ...
  const handleSignOneClick = () => {
    setDoorImage(`${process.env.PUBLIC_URL}/assets/General/Doors/unit4/bg-doors-1opened.jpg`);
    setOpeningSign(1);
    sessionStorage.setItem('currentChapter', JSON.stringify({ name: 'unitFour-first', state: 'not started' }));
    setTimeout(() => navigate('/Sub1Legal'), 800);
  };

  const handleSignTwoClick = () => {
    setDoorImage(`${process.env.PUBLIC_URL}/assets/General/Doors/unit4/bg-doors-2opened.jpg`);
    setOpeningSign(2);
    sessionStorage.setItem('currentChapter', JSON.stringify({ name: 'unitFour-second', state: 'not started' }));
    setTimeout(() => navigate('/ExplainationRTE'), 800);
  };

  const handleSignThreeClick = () => {
    setDoorImage(`${process.env.PUBLIC_URL}/assets/General/Doors/unit4/bg-doors-3opened.jpg`);
    setOpeningSign(3);
    sessionStorage.setItem('currentChapter', JSON.stringify({ name: 'unitFour-third', state: 'not started' }));
  ...
  return (
    <div className='IntroUnitFour'>
      ...
      <div className='door-signs-UnitFour'>
        {openingSign !== 1 && (
          <div className='door-sign-UnitFour-first' onClick={!openingSign ? handleSignOneClick : undefined} style={{ cursor: 'pointer' }}>
            <p className='door-sign-UnitFour-title-first'>מצבים משפטיים</p>
            <img src={`${process.env.PUBLIC_URL}/assets/General/Doors/DoorsSigns/DoorSignOne.png`} alt="Sign 1" class="title-img"/>
          </div>
        )}
        {openingSign !== 2 && (
          <div className={`door-sign-UnitFour-second ${!canEnterSecond ? 'disabled' : ''}`} onClick={!openingSign && canEnterSecond ? handleSignTwoClick : undefined}>
            <p className='door-sign-UnitFour-title-second'>מעבר משגרה לחירום</p>
            <img src={`${process.env.PUBLIC_URL}/assets/General/Doors/DoorsSigns/DoorSignTwo.png`} alt="Sign 2" class="title-img"/>
          </div>
        )}
        {openingSign !== 3 && (
          <div className={`door-sign-UnitFour-third ${!canEnterThird ? 'disabled' : ''}`} onClick={!openingSign && canEnterThird ? handleSignThreeClick : undefined}>
            <p className='door-sign-UnitFour-title-third'>אירוע חירום</p>
            <img src={`${process.env.PUBLIC_URL}/assets/General/Doors/DoorsSigns/DoorSignFour.png`} alt="Sign 3" class="title-img"/>
          </div>
        )}
      </div>

      {/* תמונות ה-V */}
      {finishedChapters.unitFourFirst && <img className='doorOneDone IntroUnitFour' src={`${process.env.PUBLIC_URL}/assets/General/Doors/DoorsDone/DoorTwoDone.png`} alt="1" />}
      {finishedChapters.unitFourSecond && <img className='doorTwoDone IntroUnitFour' src={`${process.env.PUBLIC_URL}/assets/General/Doors/DoorsDone/DoorTwoDone.png`} alt="2" />}
      {finishedChapters.unitFourThird && <img className='doorThreeDone IntroUnitFour' src={`${process.env.PUBLIC_URL}/assets/General/Doors/DoorsDone/DoorThreeDone.png`} alt="3" />}

      {/* כפתורי קיצור - למחוק אחר כך */}
      <button className='ShortcutButton' id='ShortcutButton1-four' onClick={() => {
        sessionStorage.setItem('unitFour-first', 'finished');
        setFinishedChapters(prev => ({ ...prev, unitFourFirst: true }));
      }}>כפתור קיצור- דלת 1</button>
      <button className='ShortcutButton' id='ShortcutButton2-four' onClick={() => {
        sessionStorage.setItem('unitFour-second', 'finished');
        setFinishedChapters(prev => ({ ...prev, unitFourSecond: true }));
      }}>כפתור קיצור- דלת 2</button>
      <button className='ShortcutButton' id='ShortcutButton3-four' onClick={() => {
        sessionStorage.setItem('unitFour-third', 'finished');
        setFinishedChapters(prev => ({ ...prev, unitFourThird: true }));
      }}>כפתור קיצור- דלת 3</button>

      {/* מודל סיום */}
      {showEndModal && (
        <div className="questions-end-overlay">
          <div className="questions-end-modal" style={{ borderColor: modalBorderColor }}>
